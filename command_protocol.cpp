 ...
 *
 * 线程安全设计:
 *   - WebSocket 回调 (Core 0): 只调用 enqueueCommand() 入队
 *   - loop() (Core 1): 调用 processCommandQueue() 出队并执行
 *   - 所有状态读写都在 Core 1 上串行化, 无需互斥锁
 ******************************************************************************/
...
bool enqueueCommandClient(const char* json, uint32_t clientId) {
  ...
  BaseType_t ret = xQueueSend(cmdQueue, &msg, 0);  // 非阻塞
  return (ret == pdTRUE);
}
...
void processCommandQueue() {
  ...
  while (xQueueReceive(cmdQueue, &msg, 0) == pdTRUE) {
    ...
    // 如果有 WebSocket 客户端关联且注册了回调, 将响应发回
...
// 静态响应缓冲区 (parseAndExecute 返回指针指向这里)
