...
static void tick_running() {
  ...
  if (pump.mode == MODE_JET) {
    if (pump.jetSquirting) {
      ...
      if (pump.jetInterval > 15 && pump.stepperEnabled) { stepper->disableOutputs(); pump.stepperEnabled = false; beepDisable(); }
      if (!pump.stepperEnabled && pump.jetInterval > 15 && elapsed >= intervalMs - 2000) { stepper->enableOutputs(); pump.stepperEnabled = true; }
  ...
  else if (millis() - pump.stallCheckTime > STALL_TIMEOUT_MS) { stepper->forceStop(); stepper->disableOutputs(); pump.stepperEnabled = false; pump_machine_transition(STALL_ERROR); }
}
